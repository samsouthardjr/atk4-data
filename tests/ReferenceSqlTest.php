...
class ReferenceSqlTest extends TestCase
{
    ...
    public function testAggregateHasMany(): void
    {
        ...
        $l = (new Model($this->db, ['table' => 'invoice_line']))
            ->addField('invoice_id')
            ->addField('total_net', ['type' => 'atk4_money'])
            ->addField('total_vat', ['type' => 'atk4_money'])
            ->addField('total_gross', ['type' => 'atk4_money']);
        $i->hasMany('line', ['model' => $l])
    ...
    public function testOtherAggregates(): void
    {
        ...
        $l->hasMany('Items', ['model' => $i])
            ->addField('items_name', ['aggregate' => 'count', 'field' => 'name'])
            ->addField('items_code', ['aggregate' => 'count', 'field' => 'code']) // counts only not-null values
            ->addField('items_star', ['aggregate' => 'count']) // no field set, counts all rows with count(*)
            ->addField('items_c:', ['concat' => '::', 'field' => 'name'])
            ->addField('items_c-', ['aggregate' => $i->dsql()->groupConcat($i->expr('[name]'), '-')])
            ->addField('len', ['aggregate' => $i->expr($buildSumWithIntegerCastSqlFx($buildLengthSqlFx('[name]')))]) // TODO cast should be implicit when using "aggregate", sandpit http://sqlfiddle.com/#!17/0d2c0/3
            ->addField('len2', ['expr' => $buildSumWithIntegerCastSqlFx($buildLengthSqlFx('[name]'))])
            ->addField('chicken5', ['expr' => $buildSumWithIntegerCastSqlFx('[]'), 'args' => ['5']]);
    ...
    public function testHasOneTitleSet(): void
    {
        ...
        $u = (new Model($this->db, ['table' => 'user', 'title_field' => 'last_name']))
            ->addField('name')
            ->addField('last_name');
        $o = (new Model($this->db, ['table' => 'order']));
        $o->hasOne('user_id', ['model' => $u])->addTitle();
    ...
    public function testHasOneReferenceType(): void
    {
        ...
        $user = (new Model($this->db, ['table' => 'user']))
            ->addField('name')
            ->addField('last_name')
            ->addField('some_number')
            ->addField('some_other_number');
